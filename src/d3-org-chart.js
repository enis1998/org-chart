!(function (t, e) {
  'object' == typeof exports && 'undefined' != typeof module
    ...
    : e((t.d3 = t.d3 || {}), t.d3, t.d3, t.d3, t.d3, t.d3, t.d3, t.d3);
})(this, function (t, e, a, n, o, d, i, s) {
  'use strict';
  const M = {
    ...
    stratify: n.stratify,
    zoom: o.zoom,
    zoomIdentity: o.zoomIdentity,
    linkHorizontal: i.linkHorizontal,
    drag: s.drag,
    data: null,
    sourceNode: null,
    targetNode: null,
  ...
  (t.OrgChart = class {
    constructor() {
      const a = {
          ...
          return `<defs>
                  ${t
                    .map((t) => {
                      ...
                      return `
                     <marker id="${t.from + '_' + t.to}" refX="${
                        t._source.x < t._target.x ? -7 : 7
                      }" refY="5" markerWidth="500"  markerHeight="500"  orient="${
                        t._source.x < t._target.x
                          ? 'auto'
                          : 'auto-start-reverse'
                      }" >
                     <rect rx=0.5 width=${
                    ...
                    .join(
                      (enter) =>
                        enter
                          ...
                          .call(drag(attrs))
                          .call((g) =>
                            g.append('circle').attr('r', attrs.nodeSize)
                          )
                          .call((g) =>
                            g
                              .append('text')
        ...
        linkUpdate: function (t, e, a) {
          M.select(this)
            .attr('stroke', (t) =>
              t.data._upToTheRootHighlighted ? '#152785' : 'lightgray'
            )
            .attr('stroke-width', (t) =>
              t.data._upToTheRootHighlighted ? 5 : 2
            ),
        ...
        nodeContent: (
          t
        ) => `<div style="padding:5px;font-size:10px;">Sample Node(id=${t.id}), override using <br/> <br/> 
          <code>chart<br/>
          &nbsp;.nodeContent({data}=>{ <br/>
        ...
        layoutBindings: {
          ...
          top: {
            ...
            buttonX: (t) => t.width / 2,
            buttonY: (t) => t.height,
            centerTransform: ({ rootMargin: t, scale: e, centerX: a }) =>
            ...
            swap: (t) => {},
            nodeUpdateTransform: ({ x: t, y: e, width: a }) =>
              `translate(${t - a / 2},${e})`,
          },
          bottom: {
            nodeLeftX: (t) => -t.width / 2,
    ...
    render() {
      ...
      if (!i.data || 0 == i.data.length)
        return console.log('ORG CHART - Data is empty'), this;
      const t = M.select(i.container);
      var e = t.node().getBoundingClientRect();
      0 < e.width && (i.svgWidth = e.width);
      const a = {
        id: `ID${Math.floor(1e6 * Math.random())}`,
        chartWidth: i.svgWidth,
        chartHeight: i.svgHeight,
      };
      if (
        ((i.calc = a),
        (a.centerX = a.chartWidth / 2),
      ...
      (i.flexTreeLayout = d
        .flextree({
          nodeSize: (t) => {
            ...
            return i.layoutBindings[i.layout].nodeFlexSize({
              state: i,
              node: t,
              width: e,
              height: a,
        ...
        .spacing((t, e) =>
          t.parent == e.parent ? 0 : i.neightbourMargin(t, e)
        )),
        this.setLayouts({ expandNodesFirst: !1 });
      const n = t
        .patternify({ tag: 'svg', selector: 'svg-chart-container' })
        .style('background-color', i.backgroundColor)
        .attr('width', i.svgWidth)
        .attr('height', i.svgHeight)
        .attr('font-family', i.defaultFont);
      i.firstDraw &&
        n.call(i.zoomBehavior).on('dblclick.zoom', null).attr('cursor', 'move'),
        (i.svg = n);
      const o = n.patternify({ tag: 'g', selector: 'chart' });
      return (
        (i.centerG = o.patternify({ tag: 'g', selector: 'center-group' })),
        (i.linksWrapper = i.centerG.patternify({
          tag: 'g',
          selector: 'links-wrapper',
        })),
        (i.nodesWrapper = i.centerG.patternify({
          tag: 'g',
          selector: 'nodes-wrapper',
        })),
        (i.connectionsWrapper = i.centerG.patternify({
          tag: 'g',
          selector: 'connections-wrapper',
        })),
        (i.defsWrapper = n.patternify({ tag: 'g', selector: 'defs-wrapper' })),
        i.firstDraw &&
          i.centerG.attr('transform', () =>
            i.layoutBindings[i.layout].centerTransform({
              centerX: a.centerX,
              centerY: a.centerY,
              scale: i.lastTransform.k,
    ...
    dragged(d, event) {
      ...
      M.selectAll('g.node:not(.dragging)')
        .filter((d, i) => {
          const cPInner = {
            x0: d.x,
            y0: d.y,
    ...
    dragended(d) {
      ...
      if (M.sourceNode && M.targetNode) {
        ...
        const sourceNodeIndex = M.i.data.findIndex(
          (d) => d.id == sourceNodeData.id
        );
        const targetNodeIndex = M.i.data.findIndex(
          (d) => d.id == targetNodeData.id
        );
        ...
        M.i.onDrop({
          source: M.i.data[sourceNodeIndex],
          target: M.i.data[targetNodeIndex],
        });
      }
      // clear current state
      M.sourceNode = null;
                ...
                i = this.groupBy(
                  ...
                  (t) =>
                    M.max(
                      t,
                      (t) =>
                        d.layoutBindings[d.layout].compactDimension.sizeRow(t) +
                        d.compactMarginBetween(t)
                    )
                ),
                r = M.sum(i.map((t) => t[1]));
              n.forEach((t) => {
                (t.firstCompactNode = n[0]),
                  t.firstCompact
                    ? (t.flexCompactDim = [
                        o + d.compactMarginPair(t),
                        r - d.compactMarginBetween(t),
                      ])
            ...
            a.forEach((t, e, a) => {
              0 == e && (n.x -= n.flexCompactDim[0] / 2),
                e & ((e % 2) - 1)
                  ? (t.x =
                      n.x +
                      0.25 * n.flexCompactDim[0] -
                      d.compactMarginPair(t) / 4)
                  : e &&
                    (t.x =
                      n.x +
                      0.75 * n.flexCompactDim[0] +
                      d.compactMarginPair(t) / 4);
            });
    ...
    update({ x0: a, y0: n, x: o = 0, y: i = 0, width: r, height: d }) {
      ...
      const C = s.nodesWrapper
        ...
        b = C.enter()
          ...
          .attr('transform', (t) => {
            return t == s.root
              ? `translate(${a},${n})`
              : `translate(${s.layoutBindings[s.layout].nodeJoinX({
                  x: a,
                  y: n,
                  width: r,
                  height: d,
      ...
      k
        ...
        .attr('opacity', 0)
        .duration(s.duration)
        .attr('transform', ({ x: t, y: e, width: a, height: n }) =>
    ...
    diagonal(t, e, a) {
      ...
      return `
                ...
                L ${n} ${o}
                L ${n} ${o + e * t}
                C  ${n} ${o + e * t + h * t} ${n} ${o + e * t + h * t} ${
        n + h * l
      } ${o + e * t + h * t}
                L ${n + a * l + h * l} ${o + e * t + h * t}
                C  ${i}  ${o + e * t + h * t} ${i}  ${o + e * t + h * t} ${i} ${
        r - e * t
      }
                L ${i} ${r}
     `;
    ...
    onButtonClick(t, e) {
      this.getChartState().setActiveNodeCentered &&
    ...
    setLayouts({ expandNodesFirst: t = !0 }) {
      ...
      (i.root = M.stratify()
        ...
        i.root.children && t && i.root.children.forEach(this.expand),
        i.root.children.forEach((t) => this.collapse(t)),
        0 == i.expandLevel &&
          ((i.root._children = i.root.children), (i.root.children = null)),
        [i.root].forEach((t) => this.expandSomeNodes(t));
    }
    collapse(t) {
      t.children &&
      ...
      return (
        n
          ...
          : console.log(
              `ORG CHART - ${
                t ? 'EXPAND' : 'COLLAPSE'
              } - Node with id (${e})  not found in the tree`
            ),
        this
      );
    }
    setCentered(e) {
      ...
      return (
        t
          ? ((t.data._centered = !0), (t.data._expanded = !0))
          : console.log(
              `ORG CHART - CENTER - Node with id (${e}) not found in the tree`
            ),
        this
      );
    }
    setHighlighted(e) {
      ...
      return (
        t
          ? ((t.data._highlighted = !0),
            (t.data._expanded = !0),
            (t.data._centered = !0))
          : console.log(
              `ORG CHART - HIGHLIGHT - Node with id (${e})  not found in the tree`
            ),
        this
      );
    }
    setUpToTheRootHighlighted(e) {
      ...
      return (
        t
          ? ((t.data._upToTheRootHighlighted = !0),
            (t.data._expanded = !0),
            t.ancestors().forEach((t) => (t.data._upToTheRootHighlighted = !0)))
          : console.log(
              `ORG CHART - HIGHLIGHTROOT - Node with id (${e}) not found in the tree`
            ),
        this
      );
    }
    ...
    fullscreen(t) {
      ...
      M.select(document).on('fullscreenchange.' + e.id, function (t) {
        (document.fullscreenElement ||
          ...
          document.webkitFullscreenElement) == a
          ? setTimeout((t) => {
    ...
    downloadImage({
      ...
      if (a) {
        ...
        return (
          h(
            (d = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(d)),
            'graph.svg'
          ),
          void o()
        );
      ...
      l.onload = function () {
        ...
        (a.fillStyle = '#FAFAFA'),
          a.fillRect(0, 0, e.width * s, e.height * s),
          a.drawImage(l, 0, 0, e.width * s, e.height * s);
